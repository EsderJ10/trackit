// Pure progression engine: a scheme + persisted state → next session's suggested
// sets, advanced once a session is logged. Weights are canonical kg. Schemes: lp
// (linear), dp (double), percent (training max), rpe (autoregulated e1RM).
...
/**
 * Advance the cursor one day: days rotate within a week, weeks within a cycle;
 * wrapping the last week bumps the cycle (new program pass — where percent
 * schemes bump their training max). `dayCount`/`weekCount` are program totals.
 */
...
/**
 * RTS (Tuchscherer) @RPE-10 column: %1RM for `n` reps to failure. Keys are
 * reps-to-failure × 2 (0.5-rep resolution). Transcribed from the RTS chart, not
 * Epley-inverse — `[2]=1.0` fixes the old `rpePct(10,1)=0.968` under-load bug.
 * The full chart is recovered via the RIR-shift identity (RPE 9 ≡ 1 RIR). See
 * research.txt Part 2 §A2.
 */
...
// Single source of truth shared by `rpePct` and inverse `e1rmFromLoggedSet`, so
// they can't drift. Interpolates the @10 curve at half-rep resolution.
function repsToFailurePct(n: number): number {
  ...
  if (key <= RPE10_MAX_KEY) {
    ...
    // Off-grid (non-0.5 RPE input): interpolate neighbours.
...
/**
 * Fraction of est. 1RM a set of `reps` @ `rpe` represents. RIR = 10 − RPE, so
 * it's `reps + RIR` reps from failure. RPE clamped to [1, 10]; `rpePct(10,1)===1`.
 */
...
/**
 * Est. 1RM (kg) from a logged set — the exact inverse of the rpe render path
 * (`weight = e1rm · rpePct`), so hitting the prescription holds the anchor flat,
 * beating it raises it, missing it lowers it. A to-failure estimate here would
 * spiral a hit-the-target session's anchor downward.
 */
...
/** Render one prescription into a concrete suggested set against the context. */
export function renderPrescribedSet(
  ...
  // `abs`: literal kg, falling back to the working weight when unset (0).
  return {
    reps: p.reps,
...
/**
 * Sets to pre-fill for the next session: `targetSets` identical sets at the
 * scheme's current weight and rep target. Advisory — the user can override.
 */
...
// Success = ≥ `targetSets` sets at the rep target and ≥ the working weight.
function hitTargets(
  logged: LoggedSet[],
  targetSets: number,
  targetReps: number,
...
function advanceLp(
  ...
  if (success) {
    // start + n×increment is loadable by construction; only deload needs rounding.
...
function advanceDp(
  ...
  // Hit the current target but not the top → climb one rep, same weight.
  if (
...
/**
 * One program-exercise slot's progression rule, as the DB row carries it.
 * `percent` schemes bump their TM on the cycle wrap, not per session, so they
 * never reach `advanceSlot`.
 */
...
/** A folded session's mutation: `rpe` re-anchors e1RM, `lp`/`dp` advance state. */
 ...
 * behind `advanceProgram`). Callers pass only slots with ≥1 logged working set.
 *
 * For `rpe`, re-anchor e1RM from the best logged set. Pre-filled sets log no
 * RPE, so each re-anchors at the RPE it was *rendered* at (`prescribedRpeBySet`),
 * falling back to the target — using the target on a week whose prescribed RPE
 * differs would drift the anchor.
 */
...
/**
 * Two-axis mesocycle (RP): across accumulation weeks RIR descends (effort
 * climbs) while working-set count ramps MEV→MRV; an optional lighter deload week
 * is appended. See research.txt Part 2 §A3 and 5day-ulppl spec.
 */
...
/**
 * Expand `WaveRules` into one `WaveSet` per (week, set). The deload week, if
